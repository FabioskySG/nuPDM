...
from config_nusc import GlobalConfigNusc
...
class AutoPilot(autonomous_agent_local.AutonomousAgent):
    ...
    def setup(self, path_to_conf_file, route_index=None, traffic_manager=None):
        ...
        if os.environ.get("SAVE_PATH", None) is not None:
            ...
            # self.route_index is routes_training_route0_03_05_08_42_15
            # split by _ and get the route0 part
            route_number = self.route_index.split("_")[2]
            string = f"route_{route_number}_" + pathlib.Path(os.environ["ROUTES"]).stem + "_date_"

            # string = pathlib.Path(os.environ["ROUTES"]).stem + "_test_"
    ...
    def toggle_recording(self, force_stop=False):
        ...
        if self.recording and not force_stop:
            ...
            print(f"Scenario name: {scenario_name}, Scenario number: {scenario_number}")
    ...
    def save(self, target_point, next_target_point, steering, throttle, brake, control_brake, target_speed, speed_limit,
        ...
        data = {
            ...
            "traffic_light_hazard": bool(self.traffic_light_hazard), # MODIFIED
        ...
        # print(f"Route name: {route_config.name}")
        # print(f"Scenario names: {route_config.scenario_configs[0].scenario_names}")
        # print(f"Trigger points: {route_config.scenario_configs[0].trigger_points}")

        # check if the current pos is +-3 meters from the trigger point
        # for i, tp in enumerate(route_config.scenario_configs[0].trigger_points):
        #     # import pdb; pdb.set_trace()
        #     x = tp.location.x
        #     y = tp.location.y
        #     if np.linalg.norm(np.array([x, y]) - np.array(ego_position[:2])) < 3.0:
        #         print(f"Trigger point {i} reached!")
