...
class DataAgent(AutoPilot):
    ...
    def setup(self, path_to_conf_file, route_index=None, traffic_manager=None):
        ...
        if self.save_path is not None and self.datagen:
            ...
            (self.save_path / 'CAM_FRONT').mkdir()
            (self.save_path / 'CAM_FRONT_LEFT').mkdir()
            (self.save_path / 'CAM_FRONT_RIGHT').mkdir()
            ...
            (self.save_path / 'lidar_semantic').mkdir()
    ...
    def tick(self, input_data):
        ...
        if self.save_path is not None and (self.datagen or self.tmp_visu):
            cam_front = input_data['CAM_FRONT'][1][:, :, :3]
            cam_front_left = input_data['CAM_FRONT_LEFT'][1][:, :, :3]
            cam_front_right = input_data['CAM_FRONT_RIGHT'][1][:, :, :3]
        ...
        if self.tmp_visu: # I think this is not important
    ...
    def save_sensors(self, tick_data):
        ...
        cv2.imwrite(str(self.save_path / 'CAM_FRONT' / (f'{frame:04}.jpg')), tick_data['CAM_FRONT'])
        cv2.imwrite(str(self.save_path / 'CAM_FRONT_RIGHT' / (f'{frame:04}.jpg')), tick_data['CAM_FRONT_RIGHT'])
        cv2.imwrite(str(self.save_path / 'CAM_FRONT_LEFT' / (f'{frame:04}.jpg')), tick_data['CAM_FRONT_LEFT'])
        ...
        extra_dims = [
                ...
                laspy.ExtraBytesParams(name="instance_id", type=np.uint32),
                laspy.ExtraBytesParams(name="semantic_tag", type=np.uint32),
            ]
        ...
        with laspy.open(self.save_path / 'lidar_semantic' / (f'{frame:04}.laz'), mode='w', header=header) as writer:
            ...
            point_record["instance_id"] = tick_data['lidar_semantic'][:, 4].flatten()
            point_record["semantic_tag"] = tick_data['lidar_semantic'][:, 5].flatten()
    ...
    def build_intrinsic_matrix(self, fov, w=1600, h=900, is_behind_camera=False):
    ...
    def build_extrinsic_matrix(self, x, y, z, roll, pitch, yaw):
    ...
    def get_bounding_boxes(self, lidar=None):
        ...
        cam_front_K = self.build_intrinsic_matrix(self.config.cameras_fov)
        cam_front_right_K = self.build_intrinsic_matrix(self.config.cameras_fov)
        cam_front_left_K = self.build_intrinsic_matrix(self.config.cameras_fov)
        cam_back_K = self.build_intrinsic_matrix(self.config.camera_back_fov)
        cam_back_right_K = self.build_intrinsic_matrix(self.config.cameras_fov)
        cam_back_left_K = self.build_intrinsic_matrix(self.config.cameras_fov)

        cam_front_T = self.build_extrinsic_matrix(self.config.camera_front_pos[0], self.config.camera_front_pos[1], self.config.camera_front_pos[2], self.config.camera_front_rot[0], self.config.camera_front_rot[1], self.config.camera_front_rot[2])
        cam_front_right_T = self.build_extrinsic_matrix(self.config.camera_front_right_pos[0], self.config.camera_front_right_pos[1], self.config.camera_front_right_pos[2], self.config.camera_front_right_rot[0], self.config.camera_front_right_rot[1], self.config.camera_front_right_rot[2])
        cam_front_left_T = self.build_extrinsic_matrix(self.config.camera_front_left_pos[0], self.config.camera_front_left_pos[1], self.config.camera_front_left_pos[2], self.config.camera_front_left_rot[0], self.config.camera_front_left_rot[1], self.config.camera_front_left_rot[2])
        cam_back_T = self.build_extrinsic_matrix(self.config.camera_back_pos[0], self.config.camera_back_pos[1], self.config.camera_back_pos[2], self.config.camera_back_rot[0], self.config.camera_back_rot[1], self.config.camera_back_rot[2])
        cam_back_right_T = self.build_extrinsic_matrix(self.config.camera_back_right_pos[0], self.config.camera_back_right_pos[1], self.config.camera_back_right_pos[2], self.config.camera_back_right_rot[0], self.config.camera_back_right_rot[1], self.config.camera_back_right_rot[2])
        cam_back_left_T = self.build_extrinsic_matrix(self.config.camera_back_left_pos[0], self.config.camera_back_left_pos[1], self.config.camera_back_left_pos[2], self.config.camera_back_left_rot[0], self.config.camera_back_left_rot[1], self.config.camera_back_left_rot[2])
        ...
        result = {
                ...
                # INTRINSECA Y EXTRINSECA DE CADA CAMARA
                'cam_front_K': cam_front_K.tolist(),
                'cam_front_T': cam_front_T.tolist(),
                'cam_front_right_K': cam_front_right_K.tolist(),
                'cam_front_right_T': cam_front_right_T.tolist(),
                'cam_front_left_K': cam_front_left_K.tolist(),
                'cam_front_left_T': cam_front_left_T.tolist(),
                'cam_back_K': cam_back_K.tolist(),
                'cam_back_T': cam_back_T.tolist(),
                'cam_back_right_K': cam_back_right_K.tolist(),
                'cam_back_right_T': cam_back_right_T.tolist(),
                'cam_back_left_K': cam_back_left_K.tolist(),
                'cam_back_left_T': cam_back_left_T.tolist(),
                # W2C DE CADA CAMARA
        }
