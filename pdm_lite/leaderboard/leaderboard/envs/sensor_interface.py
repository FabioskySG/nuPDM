...
class CallBack(object):
    ...
    def __call__(self, data):
        if isinstance(data, carla.libcarla.Image):
            ...
            self._parse_lidar_semantic_cb(data, self._tag)
    ...
    # def _parse_lidar_semantic_cb(self, lidar_data, tag):
    #     # Primero leemos todo como 1D array y luego lo dividimos
    #     raw_array = np.frombuffer(lidar_data.raw_data, dtype=np.dtype('f4'))
    #     # Reorganizamos en array de 6 columnas
    #     temp_array = raw_array.reshape(-1, 6)
    #     print(f"Unique raw: {np.unique(temp_array[:, 5])}")
    #     # Separamos los float32 y uint8
    #     points = temp_array[:, :4]  # Primeras 4 columnas como float32
    #     semantic_info = temp_array[:, 4:].copy()  # Hacemos una copia contigua
    #     semantic_info = semantic_info.view(np.dtype('uint8')).reshape(-1, 8)[:, :2]  # Últimas 2 columnas como uint8
    #     # Concatenamos
    #     pcd = np.concatenate((points, semantic_info), axis=1)
    #     pcd = copy.deepcopy(pcd)
    #     print(f"Unique semantic: {np.unique(pcd[:, 5])}")
    #     self._data_provider.update_sensor(tag, pcd, lidar_data.frame)

    def _parse_lidar_semantic_cb(self, lidar_data, tag):
        points = np.frombuffer(lidar_data.raw_data, dtype=np.dtype([
                ('x', np.float32), ('y', np.float32), ('z', np.float32),
                ('cos_angle', np.float32), ('obj_idx', np.uint32), ('sem_tag', np.uint32)
            ]))
        # Convertimos a un array con las columnas separadas
        points_array = np.empty((len(points), 6), dtype=np.float32)
        points_array[:, 0] = points['x']
        points_array[:, 1] = points['y']
        points_array[:, 2] = points['z']
        points_array[:, 3] = points['cos_angle']
        points_array[:, 4] = points['obj_idx']
        points_array[:, 5] = points['sem_tag']
        self._data_provider.update_sensor(tag, points_array, lidar_data.frame)
